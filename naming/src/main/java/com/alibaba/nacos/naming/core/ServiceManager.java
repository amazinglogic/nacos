...
public class ServiceManager implements RecordListener<Service> {
    ...
    public Map<String, Object> getDubboAppInfo(String namespace, String application) {
        boolean namespaceFilter = StringUtils.isNotBlank(namespace);
        boolean applicationFilter = StringUtils.isNotBlank(application);
        Set<Map.Entry<String, Map<String, Service>>> entries = serviceMap.entrySet();
        Map<String, Object> map = new LinkedHashMap<>();
        for (Map.Entry<String, Map<String, Service>> entry : entries) {
            String key = entry.getKey();
            if (namespaceFilter && !key.equals(namespace)) {
                continue;
            }
            Collection<Service> values = entry.getValue().values();
            if (CollectionUtils.isEmpty(values)) {
                continue;
            }
            List<Instance> instanceList = values.stream().flatMap(s -> s.allIPs().stream()).collect(Collectors.toList());
            long dubboTotal = instanceList.stream().filter(s -> Optional.ofNullable(s.getMetadata()).orElse(Collections.emptyMap()).containsKey("dubbo")).count();
            long dubboConsumer = instanceList.stream().filter(s -> {
                Map<String, String> metadata = Optional.ofNullable(s.getMetadata()).orElse(Collections.emptyMap());
                return metadata.containsKey("dubbo") && "consumer".equals(metadata.get("side"));
            }).count();
            Set<String> applicationSet = instanceList.stream().filter(i -> i.getMetadata().containsKey("dubbo")).map(i -> i.getMetadata().get("application")).collect(Collectors.toSet());
            //排序方便对比各环境服务
            Map<String, List<Instance>> collect = instanceList.stream().filter(i -> i.getMetadata().containsKey("dubbo")).collect(Collectors.groupingBy(s -> s.getMetadata().get("application")));

            ArrayList<String> applicationList = new ArrayList<>(applicationSet);
            applicationList.sort(String::compareTo);
            LinkedList<Map<String, Object>> applicationInfo = new LinkedList<>();
            for (String s : applicationList) {
                if (applicationFilter && !s.contains(application)) {
                    continue;
                }
                List<Instance> instances = collect.get(s);
                Map<String, List<Instance>> ipMap = instances.stream().collect(Collectors.groupingBy(com.alibaba.nacos.api.naming.pojo.Instance::getIp));

                for (Map.Entry<String, List<Instance>> stringListEntry : ipMap.entrySet()) {
                    HashMap<String, Object> info = new LinkedHashMap<>();
                    info.put("name", s);
                    info.put("ip", stringListEntry.getKey());
                    List<Instance> value = stringListEntry.getValue();
                    info.put("totalCount", value.size());
                    info.put("providerCount", value.stream().filter(x -> "provider".equals(x.getMetadata().get("side"))).count());
                    info.put("consumerCount", value.stream().filter(x -> "consumer".equals(x.getMetadata().get("side"))).count());
                    long healthyCount = value.stream().filter(com.alibaba.nacos.api.naming.pojo.Instance::isHealthy).count();
                    info.put("healthyCount", healthyCount);
                    info.put("healthy", healthyCount == value.size());
                    info.put("apiVersion", value.stream().filter(x -> StringUtils.isNotBlank(x.getMetadata().get("apiVersion"))).map(x -> x.getMetadata().get("apiVersion")).findFirst().orElse("default"));
                    info.put("enabled", value.stream().filter(Instance::isEnabled).findAny());
                    applicationInfo.add(info);
                }
            }
            map.put("namespace", namespace);
            map.put("serviceTotal", dubboTotal);
            map.put("total", instanceList.size());
            map.put("consumer", dubboConsumer);
            map.put("provider", dubboTotal - dubboConsumer);
            map.put("applicationCount", applicationList.size());
            map.put("applicationList", applicationInfo);
        }
        return map;
    }

    public Map<String, Object> getDubboServices(String namespace, String application, String ip) {
        Set<Map.Entry<String, Map<String, Service>>> entries = serviceMap.entrySet();
        Map<String, Object> map = new LinkedHashMap<>();
        for (Map.Entry<String, Map<String, Service>> entry : entries) {
            String key = entry.getKey();
            if (!Objects.equals(key, namespace)) {
                continue;
            }
            Collection<Service> values = entry.getValue().values();
            if (CollectionUtils.isEmpty(values)) {
            ...
            List<Map<String, Object>> instanceList = values.stream().flatMap(s -> s.allIPs().stream())
                .filter(s -> (realServiceName(s.getServiceName()).equals(application)
                    || Objects.equals(s.getMetadata().get("application"), application))
                    && Objects.equals(s.getIp(), ip))
                .flatMap(instance -> {
                    HashMap<String, Object> info = new HashMap<>(4, 1);
                    String serviceName = instance.getServiceName();
                    String[] split = serviceName.split(SERVICE_NAME_SEPARATOR);
                    info.put("name", split[1]);
                    info.put("group", split[0]);
                    info.put("lastBeat", DateFormatUtils.format(instance.getLastBeat(),"yyyy-MM-dd HH:mm:ss"));
                    info.put("healthy", instance.isHealthy());
                    info.put("enabled", instance.isEnabled());
                    return Stream.of(info);
                })
                .collect(Collectors.toList());
            map.put("serviceList", instanceList);
            map.put("serverTime", DateFormatUtils.format(System.currentTimeMillis(),"yyyy-MM-dd HH:mm:ss"));
        }
        return map;
    }

    @SuppressWarnings("DuplicatedCode")
    public Map<String, Object> getDubboAppInfoDetail(String namespace, String application) {
        boolean namespaceFilter = StringUtils.isNotBlank(namespace);
        boolean applicationFilter = StringUtils.isNotBlank(application);
        Set<Map.Entry<String, Map<String, Service>>> entries = serviceMap.entrySet();
        Map<String, Object> map = new LinkedHashMap<>();
        ...
        for (Map.Entry<String, Map<String, Service>> entry : entries) {
            ...
            if (applicationFilter) {
                List<String> targetList = applicationList.stream().filter(s -> s.contains(application)).collect(Collectors.toList());
                targetList.forEach(s -> {
                    Set<String> healthyIps = collect.get(s).stream().filter(com.alibaba.nacos.api.naming.pojo.Instance::isHealthy).map(Instance::getIp).collect(Collectors.toSet());
                    Set<String> unhealthyIps = collect.get(s).stream().filter(x -> !x.isHealthy()).map(Instance::getIp).collect(Collectors.toSet());
                    LinkedHashMap<String, Object> app = new LinkedHashMap<>();
                    app.put("name", s);
                    ArrayList<String> healthyList = new ArrayList<>(healthyIps);
                    healthyList.sort(String::compareTo);
                    ArrayList<String> unhealthyIpList = new ArrayList<>(unhealthyIps);
                    unhealthyIpList.sort(String::compareTo);
                    app.put("heathy", healthyList);
                    app.put("unheathy", unhealthyIpList);
        ...
        map.put("count", applicationInfo.size());
        return map;
    }

    public void updateApplication(String namespaceId, String application, String ip, Boolean enabled) throws NacosException {
        Map<String, Service> map = serviceMap.get(namespaceId);
        if (CollectionUtils.isEmpty(map)) {
            return;
        }
        List<Instance> instances = map.values().stream().flatMap(s -> s.allIPs().stream()).filter(instance -> {
            Map<String, String> metaDataMap = Optional.ofNullable(instance.getMetadata()).orElse(Collections.emptyMap());

            return (realServiceName(instance.getServiceName()).equals(application) || Objects.equals(metaDataMap.get("application"), application)) &&
                Objects.equals(instance.getIp(), ip);

        }).collect(Collectors.toList());

        ObjectMapper objectMapper = new ObjectMapper();
        Map<String, String> params = new HashMap<>(10, 1);
        for (Instance instance : instances) {
            String serviceName = instance.getServiceName();
            String[] split = serviceName.split(SERVICE_NAME_SEPARATOR);
            params.put("serviceName", split[1]);
            params.put("clusterName", instance.getClusterName());
            params.put("groupName", split[0]);
            ...
            String server = distroMapper.mapSrv(instance.getServiceName());
            RestResult<String> result = HttpClient.request("http://" + server + "/nacos/v1/ns/instance", Collections.emptyList(), params, null, 10000, 10000,
                Charsets.UTF_8.name(), "PUT");
            try {
                Loggers.SRV_LOG.info("请求{}上下线服务,param:{},result:{}", server, objectMapper.writeValueAsString(params), result);
            }
            catch (Exception e) {
            }
        }
    ...
    private String realServiceName(String name) {
        if (StringUtils.isBlank(name)) {
            return name;
        }
        if (name.contains(SERVICE_NAME_SEPARATOR)) {
            String[] split = name.split(SERVICE_NAME_SEPARATOR);
            return split[1];
        }
        return name;
    }
